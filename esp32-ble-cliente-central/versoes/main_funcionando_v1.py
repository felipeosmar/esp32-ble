...
class BLESensorClient:
    def __init__(self, target_mac=None, device_name=None):
        ...
        # Configurações
        self.max_retries = 3
        self.scan_duration = 30000  # 10 segundos
    ...
    def _handle_scan_done(self, data):
        ...
        if self.target_device:
            print(f"\n>>> Conectando ao dispositivo alvo...")
...
def main():
    ...
    try:
        ...
        if client.scan_devices():
            ...
            # Se não conectou automaticamente, mostra opções
            if not client.connected:
                ...
                if client.found_devices:
                    ...
                    # Pega o primeiro dispositivo como exemplo
                    primeiro_mac = list(client.found_devices.keys())[0]
                    print(f"client.conectar_por_mac('{primeiro_mac}')")
                    print("client.conectar_por_indice(1)")
                    
                    # Conecta automaticamente ao dispositivo com melhor sinal
                    melhor_dispositivo = max(client.found_devices.items(), 
                    ...
                    if client.conectar_por_mac(melhor_mac):
                        # Aguarda conexão
                        timeout = time.ticks_ms() + 10000  # 10 segundos
                        while time.ticks_diff(timeout, time.ticks_ms()) > 0:
            ...
            # Se conectou, entra no loop principal
            if client.connected and client.char_discovered:
                print("\n" + "="*50)
                ...
                print("Não foi possível conectar e configurar o dispositivo")
